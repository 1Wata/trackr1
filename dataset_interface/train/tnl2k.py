...
from utils.utils import jpeg4py_loader_w_failsafe
import re
import json
...
class TNL2k(BaseVideoDataset):
    ...
    def _get_frame(self, seq_path, seq_name, frame_id):
        if self.return_img_path:
            return self._get_frame_path(seq_path, seq_name, frame_id)
        else:
...
class TNL2k_Lang(TNL2k):
    ...
    def get_frames(self, seq_id, frame_ids, anno=None, use_first_frame_template=False):
        ...
        if use_first_frame_template:
            frame_list.insert(0, self._get_frame(seq_path, seq_name, 0))
        if anno is None:
            anno = self.get_sequence_info(seq_id)
