...
from utils.utils import jpeg4py_loader
import os
...
class OTB_Lang(BaseVideoDataset):
    ...
    def __init__(
        self, root=None, image_loader=jpeg4py_loader, split=None, data_fraction=None
    ):
    ...
    def _read_bb_anno(self, seq_id):
        ...
        gt = load_text(
            str(bb_path), delimiter=(",", None), dtype=np.float64, backend="numpy"
        )
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(
            seq_path, "{:04}.jpg".format(frame_id + 1)
        )  # frames start from 1
    ...
    def _get_expression(self, seq_id):
        ...
        exp_path = os.path.join(
            self.nlp_root, self.sequence_list[seq_id]["name"] + ".txt"
        )
        exp_str = ""
        try:
        ...
        assert (
            exp_str != "" and not exp_str is None
        ), 'ERROR: Language File is None: "{}"'.format(exp_path)
        exp_str = clean_string(exp_str)
        return exp_str

    def get_frames(self, seq_id, frame_ids, anno=None, use_first_frame_template=True):
        seq_path = self._get_sequence_path(seq_id)

        frame_list = [self._get_frame(seq_path, seq_id, f_id) for f_id in frame_ids]
        if use_first_frame_template:
            frame_list.insert(0, self._get_frame(seq_path, seq_id, 0))
        ...
        if use_first_frame_template:
            for key, value in anno.items():
                anno_frames[key].insert(0, value[0, ...].clone())
