...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    # 使用 Matplotlib 的 savefig 保存为 PGF 格式
    pgf_filepath = '{}/{}_plot.pgf'.format(result_plot_path, plot_type)
    try:
        fig.savefig(pgf_filepath) # Matplotlib 会根据扩展名推断格式
    except Exception as e:
        print(f"警告：无法保存为 PGF 格式 ({pgf_filepath}): {e}")
        print("请确保已安装 LaTeX 和 PGF 相关依赖。")
